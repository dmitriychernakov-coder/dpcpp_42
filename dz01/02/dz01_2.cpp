...
public:
    ...
    SqlSelectQueryBuilder& AddWhere(const std::map<std::string, std::string>& kv) noexcept {
        for (const auto& data : kv) {
            zapros.push_back({data.first, data.second});
        }
        return *this;
    }
...
int main() {
    ...
    query_builder_all.AddWhere("name", "John").AddFrom("students");
    std::string result2 = query_builder_all.BuildQuery();
    std::cout << result2 << std::endl;
    assert(result2 == "SELECT * FROM students WHERE name=John;");


    SqlSelectQueryBuilder add_builder;
    add_builder.AddColumns({"name", "phone", "email"})
                    .AddFrom("students")
                    .AddWhere({{"id", "42"}, {"rating", "5"}});

    std::string result3 = add_builder.BuildQuery();
    std::cout << result3 << std::endl;
    
    assert(result3 == "SELECT name, phone, email FROM students WHERE id=42 AND rating=5;");
